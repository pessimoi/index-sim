...
| Market price sync | Local scheduled writer validates normalized fixture/input JSON, has a fixture-evidenced raw `markets.lostcity.rs` response adapter for `--upstream-url`, rejects redirects/fragments/credentials, bounds fetch plus body time and pre-parse bytes, requires JSON and can update `prices.json`, `alch.json` and `price-history.json` candidates. | Verify the exact live `markets.lostcity.rs` API/scrape response contract before relying on a production scheduled run.                     | `later`            | The target source is accepted by [D-021](../project/decisions.md), and the repo has a narrow raw adapter based on sanitized fixtures plus legacy evidence. The first-party manifest exposes `GET /api/items`, but route existence alone does not prove adapter compatibility or acceptable automated use. D-053 keeps live-shape proof out of the V1/trusted-tester gate. | Verify the live response shape with sanitized evidence before setting `MARKET_PRICES_UPSTREAM_URL`; do not store raw upstream dumps in docs or fixtures. |
...
| Hiscores          | D-061 accepts the first-party 2004Scape JSON API and the server-only provider maps its documented type 1-7 rows.                                                                                                                                                                                                                                | Keep the authoritative source contract fixture-backed and out of browser code.                                                             | `release-required` | The source/provider slice is implemented with fixed-origin, redirect, response-bound, schema and sanitized-error guards.                                                                                                                                                                                                                                                  | Keep provider tests mocked and update fixtures only against reviewed first-party contract evidence.                                                      |
