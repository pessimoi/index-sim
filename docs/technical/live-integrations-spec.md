...
Product decision: hiscores is required for v1 replacement. D-061 accepts the first-party 2004Scape JSON API as the authoritative V1 source; the final production hosting model and player-query access-log policy remain open.
...
Users should get current item prices from an approved market source and run simulations with an explicit PriceSet. High alch is Revision 274 game data, not a market value.
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Market price sync | Visible Settings/Economy loads scheduled market prices, generated high alch and read-only shared history, preserves local PriceSet/comparison controls and says refresh is scheduled.                                                                                        | Keep production copy free of user-triggered refresh promises and stale `/api/prices`, `/api/scrape` or `run_sim.py` instructions. | `release-required` | Implemented for the accepted static market slice.                                                                                                                                                                                                                                                          | Keep the release-copy audit in [testing.md](testing.md) mandatory before release.                                                               |
| Market price sync | D-062 item-page adapter, robust estimator, request bounds, history compaction and two-file commit allowlist are implemented with mocked/fixture evidence.                                                                                                                    | Confirm acceptable automated use and observe the first configured scheduled run before relying on scheduled-current prices.       | `later`            | The page shape and parser contract are now known; permission/politeness acceptance and external run evidence remain operations facts that code cannot prove. D-053 keeps them out of the trusted-tester gate.                                                                                              | Configure only the exact root URL after review; do not store raw pages, usernames or upstream dumps.                                            |
...
Decision update: this handler remains useful for mocked tests and local compatibility, but D-033 makes production prices scheduled static JSON. D-062 implements the item-page contract and robust estimator, D-063 moves high alch to generated game data and D-064 exposes static shared history read-only. D-053 keeps acceptable automated-use confirmation, exact root URL configuration and a successful scheduled run out of the V1/trusted-tester gate, but they remain required before prices can be described as scheduled-current.
...
Current implementation note: `scripts/write-scheduled-market-prices.ts` is the entrypoint, `scripts/scheduled-market-writer-core.ts` owns estimation/history and `scripts/markets-lostcity-item-page-adapter.ts` parses the first Inertia `soldListings` page for each allowlisted mapping. `--upstream-url` must be the exact root `https://markets.lostcity.rs/`; the writer derives item paths, runs requests sequentially with a 350 ms delay and drops usernames before normalized data reaches the estimator. `--input` remains the normalized fixture/dev path. Its contract is:
...
Rows are matched by approved `itemId` and `sourceSlug`; unknown, duplicate, mismatched or missing rows fail before any write. Completed buy and sell listings count as one observation each regardless of quantity. At least five observations use median absolute deviation filtering; three or four use a three-times median ratio guard. The writer averages at most the newest five accepted rows, retains the previous price below three accepted rows, ignores rows older than 90 days and retains when the newest accepted row is older than 30 days. Fixed-origin, root-path, credential/query/fragment, redirect, per-response timeout and byte limits fail closed with sanitized errors. The writer validates both candidate files in memory, writes deterministic JSON only after validation and skips unchanged files. The workflow rejects any diff outside `prices.json` and `price-history.json`. Tests are mocked/fixture-based and never call the live source.
...
Open question: acceptable automated use of the bounded item-page read has not been confirmed. Do not configure the exact root `MARKET_PRICES_UPSTREAM_URL` or claim scheduled-current prices until that review and the first successful scheduled run are recorded. The parser/page shape itself is implemented from bounded evidence; raw pages and usernames must remain uncommitted and unlogged.
...
Current implementation note: hiscores lookup is wired into the rewrite Levels preview/apply workflow. Market prices are wired through scheduled static status and local overrides, generated high alch and shared/local Economy history. Settings and Economy expose export and local reset controls; reset keeps both shared and local history. Hiscores still uses service-aware states from same-origin endpoints, and market production copy says refresh is scheduled and not user-triggered.
...
Current status: the same-origin compatibility API, scheduled-only UI, imported-price fallback, selected PriceSet persistence, generated alch authority, read-only shared history, local comparisons, item-page writer and robust estimator are implemented with mocked/fixture tests. The visible Market/Economy workflow remains `Valmis`. Full scheduled-current acceptance still requires acceptable automated-use confirmation and a successful configured GitHub Actions run.

## Open questions

- Which concrete backend/runtime should host the hiscores same-origin API?
- What is the production deployment target?
- What player-query redaction and access-log retention policy should the production Hiscores runtime enforce?
