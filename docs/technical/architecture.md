...
- Implemented: React/Vite rewrite UI using view models for combat setup, result summary, special attack controls/metrics, loot/economy, trip, final per-monster Cannon workbench tab, monster compare and planner in a selected parity slice.
- Implemented: rewrite-owned Planner UI state foundation in `src/app/state/planner.ts` with the versioned `index-sim:planner-ui` persistence contract for metric, target levels, current XP, skill locks, avg-over-session, only-current-gear and gear-pool restrictions. `src/app/view-models/simulation.ts` adapts that state into `src/domain/planner` input/options while keeping Planner UI state out of `SimulationRequest`, constrains gear-pool ids to the active non-hypothetical default planner pool and builds Planner panel data for summary, training order, unlock summary, gear timeline and DPS-vs-cumulative-XP chart output. The workbench Planner tab exposes the metric/current-XP/target/skill-lock/gear-pool/Recompute workflow, with only-current-gear mapped to the domain `lockGear` option; active avg-over-session semantics remain open because the current domain planner has no matching option.
- Implemented: browser adapters for sandboxed legacy data bootstrap, version 3 rewrite setup persistence with per-combat-type melee/ranged/magic loadout stash/restore, defaulted manual accuracy/damage/speed override state, Trip scarce/AFK target and respawn controls, rewrite-owned monster-specific custom setup snapshots, dense compare sort/filter/irrelevant-state persistence, setup export/import, versioned per-monster loot settings, legacy storage detection with safe `sim_input_v3` setup, `sim_hiscore_player` last-player and compatible legacy price/alch import reports, per-key legacy migration policy classification, `sim_planner_v1` detect/review-only handling that never imports into `index-sim:planner-ui`, user-facing import/keep/clear UX with visible review/reset details, validated price-set import, browser-local accepted-price history persistence and local Economy movers analysis/Snapshot now/Clear history behavior.
...
- Covered by tests: formulas, stance fallback, two-handed shield exclusion, thrown ammo handling, bounded manual combat override behavior, golden parity for ported combat fields across the 18 `SimEngine.simulate()` fixtures, trip/loot/supply parity across all 18 fixtures including cannon occupancy and cannonball supply plus focused scarce/respawn and inventory reserve detail coverage, planner unit tests plus 3 golden plan fixtures, Planner UI state/schema and adapter tests including gear-pool cleanup, editor options, timeline data and deterministic chart data, UI view-model tests including active per-style loadout request mapping, manual override request mapping, Trip scarce/reserve mapping, linked cannon/sparse assumptions, per-monster loot settings, dense compare filters/relevance state and custom setup dense row mapping, versioned persistence tests including per-style loadout, manual override, dense compare state, Trip scarce controls, per-monster cannon settings, per-monster loot settings and custom setup validation, legacy migration/reset tests including `sim_planner_v1` review-only/non-import behavior, local Economy movers analysis tests, Playwright smoke tests including per-style loadout restore, manual combat override reset, dense compare filter/relevance persistence, Trip scarce/prayer/food controls, final Cannon tab link/reset/output metrics, Planner tab metric/current-XP/target/skill-lock/gear-pool/Recompute/training-order/timeline/chart flow, legacy Planner state import/keep/clear boundary, per-monster loot settings persistence, Economy history controls and SetupBar custom setup create/restore/remove, schema validation for the legacy snapshot and committed price files, malformed imported price rejection, missing-price warnings, mocked live integration contract validation, mocked hiscores API/adapter/UI apply behavior and mocked market API/adapter/UI sync behavior.
- Not ported yet: full legacy UI parity, authoritative hiscores upstream provider, authoritative market upstream provider, legacy planner/custom setup/loot prefs/compare/cannon/hidden tiers and full price-history migration, and authoritative generated data workflow.
