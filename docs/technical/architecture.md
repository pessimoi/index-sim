...
- Not completed yet: Cloudflare account/Git connection and deployed Hiscores/D-065 verification; a successful configured market scheduled run; authoritative raw-source item requirement fallback removal; quest-gated/clue tertiary loot policy; and source-backed NPC-size behavior. The runtime adapter and provider-specific artifact/config tests are complete, but public live claims stay gated on deployed evidence. The market page/catalog contract, full live dry-run and exact root Actions variable are evidenced, but scheduled-current claims stay separately gated.
...
- Which custom domain, if any, should replace the accepted Cloudflare provider domain after preview/production evidence passes?
