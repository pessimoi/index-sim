...
- `scripts/write-scheduled-market-prices.ts`, `scripts/scheduled-market-writer-core.ts` and `scripts/markets-lostcity-item-page-adapter.ts`: repo-owned scheduled market writer, robust estimator and first-page Inertia adapter. `--input` keeps a normalized fixture/dev contract; `--upstream-url` must be the exact approved origin root and derives one `/items/{slug}` request per allowlisted mapping. Requests run sequentially with a 350 ms delay, bounded body/timeout/redirect checks and immediate removal of player identity fields. Completed buy and sell rows are unweighted observations; adaptive MAD/median filtering, 90/30-day freshness gates and prior-value retention produce `prices.json`. History keeps 12-hour points for 90 days and one latest point per older UTC day. The workflow commits only `prices.json` and `price-history.json`; acceptable automated use, root variable configuration and the first successful run remain required before scheduled-current claims.
...
- Implemented: shared live integration contracts, Zod validation, mocked fixture basis, the D-061 first-party 2004Scape Hiscores JSON provider behind the same-origin status/lookup boundary, a market source allowlist, same-origin market compatibility scaffolding, the visible scheduled static price/history path, D-062's item-page writer and the scheduled GitHub Actions commit-if-diff workflow. Vite dev/preview injects the fixed-origin Hiscores provider with redirect, timeout, response-size, schema, rate-limit and sanitized-error boundaries; this does not choose production hosting, add databases, add server-managed state or make live upstream calls part of automated tests.
...
- Not completed yet: production Hiscores runtime/hosting and player-query access-log policy; market automated-use acceptance plus a successful configured scheduled run; authoritative raw-source item requirement fallback removal; quest-gated/clue tertiary loot policy; and source-backed NPC-size behavior. The market page contract itself is fixture-covered from bounded live shape evidence, but production automation stays gated. D-048 keeps legacy planner state review-only/not migrated for V1, and D-049 keeps full legacy price-history payloads review-only/not migrated for V1.
...
- Target market price source is `markets.lostcity.rs`; scheduled-only GitHub Actions refresh into `prices.json` and `price-history.json` is accepted. D-062's item-page adapter and estimator plus D-064's read-only shared history path are implemented. D-053 keeps acceptable automated-use confirmation, the exact root `MARKET_PRICES_UPSTREAM_URL` value and latest successful scheduled-run evidence out of the V1/trusted-tester gate, but they remain required before scheduled-current market price claims.
