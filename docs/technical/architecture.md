...
- Current generated runtime evidence: `npm run runtime:readiness` is `ready` with zero blockers across IDs, required runtime fields, 63 monster combat/loot/size rows, 94 requirement rows and PriceSet coverage. `docs/project/revision-impact/current.md` passes 11/11 representative cases with accepted D-055/D-057/D-071 source deltas and reports 22 informational all-monster outliers. D-059 makes this generated snapshot root runtime truth; the legacy-derived bridge remains reference evidence.
