...
- `src/app/state/selected-price-set.ts`: rewrite-owned browser-local selected active `PriceSet` state in `index-sim:price-set:selected`. It restores a valid selected `PriceSet` over the bundled context on load, validates with `PriceSetSchema`, rejects oversized/invalid/version-mismatched envelopes and clears only this key on reset-to-bundled.
...
- Not ported yet: full legacy UI parity, authoritative hiscores upstream provider, scheduled market snapshot writer, legacy planner/custom setup/cannon and full price-history migration, and authoritative generated data workflow.
...
- Target generated data source is a single normalized current `GameDataSnapshot` generated from the current accepted LostCityRS/Content game revision in staged v1 scope. The accepted scope is only simulator-consumed domain data: monsters, items, equipment/weapons/ammo/spells, drop rows, planner/loadout requirements and provenance/warnings. Revision bumps are accepted only as reviewed development changes through the manual `npm run data:generate` PR workflow, but the generator implementation and committed generated files are not present yet.
- Target market price source is `markets.lostcity.rs`; scheduled-only GitHub Actions refresh into `prices.json`, `alch.json` and `price-history.json` is accepted, but the writer workflow is not present.
...
- What exact upstream response contract should the accepted GitHub Actions `markets.lostcity.rs` price snapshot writer use?
