...
- Current generated runtime evidence: `npm run runtime:readiness` is `ready` with zero blockers across IDs, required runtime fields, 63 monster combat/loot/size rows, 94 requirement rows, 25 typed conditional loot rows and PriceSet coverage. `docs/project/revision-impact/current.md` passes 11/11 representative cases with accepted D-055/D-057/D-071/D-072 source and policy deltas and reports 22 informational all-monster outliers. D-059 makes this generated snapshot root runtime truth; the legacy-derived bridge remains reference evidence.
...
- Implemented: the D-073 bounded loadout optimizer in `src/app/view-models/simulation.ts`. It receives the active form plus the UI's hidden-tier-filtered weapon/equipment option allowlists, builds a deterministic two-dimension Pareto frontier capped at 512 states per weapon and evaluates survivors through `simulateCombat()` for current-target normal DPS. The unchanged form is always a candidate, equal DPS prefers fewer changes, two-handed weapon normalization remains owned by `applyWeaponSelection()` and the UI applies or restores the complete form through existing default/custom setup ownership. It adds no request, persistence, provider or Planner state and does not read prices, quest state, requirements or future gear.
- Implemented: Stats special-attack and cannon source details now carry event-scoped hit distributions through the existing `HitDistributionViewModel`. Special uses one modeled special hit (`expPerSpec / hits`), cannon uses one fired cannonball (`cannonDps / ballsPerSec`), and both reuse `src/domain/combat.createHitDistribution()` rather than introducing parallel probability formulas. Inactive/invalid states remain `null`; no distribution state enters `SimulationRequest` or persistence. [The Stats source distribution specification](stats-source-distribution-spec.md) owns this boundary and D-069 excludes aggregate multi-hit, kill, trip and hourly distributions.
- Implemented: the root workbench keyboard contract in `src/app/App.tsx` and `src/app/styles.css`. The navigation uses one roving `tablist` over a dynamic labelled `tabpanel`, supports arrow/Home/End activation, and has a first-focus skip link. Dense monster rows use one selected-row Tab stop, arrow/Home/End focus movement and Enter/Space activation. Global `:focus-visible` styling restores an explicit non-layout-shifting outline, including file-button focus. D-070 keeps this a bounded interaction pass rather than a conformance claim; no domain or persisted state changes.
