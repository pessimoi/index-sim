...
- Not completed yet: production Hiscores runtime/hosting and deployed D-065 log-policy verification; market root-variable configuration plus a successful scheduled run; authoritative raw-source item requirement fallback removal; quest-gated/clue tertiary loot policy; and source-backed NPC-size behavior. The market page/catalog contract and full live dry-run are evidenced, but scheduled-current production claims stay gated. D-048 keeps legacy planner state review-only/not migrated for V1, and D-049 keeps full legacy price-history payloads review-only/not migrated for V1.
