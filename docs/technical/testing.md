...
The audit reads the gitignored `.sources/lostcity-content/` checkout recursively, follows config/category/delegated-handler behavior, fails duplicate config or `ai_queue3` definitions and emits repository-relative sanitized diagnostics. Current Revision 274 evidence maps all 63 runtime monsters, extracts 63/63 core-loot tables, emits size for 63/63 monsters, resolves all expected runtime catalog identities and maps 94 runtime item requirements. The generated item catalog contains 371 expected identities plus 15 source-only loot identities. Four quest-gated rows and 21 clue-scroll tertiary rows are explicit scoped exclusions. D-055, D-056, D-057 and D-071 own the accepted source/formula deltas; D-059 owns runtime consumption.
...
| `npm run test`                                              | `pass`                                                                    | The D-071 full Vitest gate passed 34 files and 528 tests, including raw levelrequire/NPC-size parsing, committed generated contract, dragon-halberd size behavior, generated Strength Planner/setup consumption and refreshed classified Planner parity evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `npm run build`                                             | `pass`                                                                    | The D-071 Vite build passed with only the known chunk-size warning. Artifact validation passed 7 files/2 assets, 1,435,627 bytes, 13 history snapshots and SHA-256 `acb785ea914a29cacbe33a8514d8a5e7be9b69c412aebbb64db75942610c2f8c`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Generated/runtime evidence                                  | `pass`                                                                    | Deterministic raw Revision 274 generation produced 386 items, 63 monsters with size and 94 numeric requirement rows. Generated and legacy-reference readiness are `ready`, coverage has no blockers, source audit has zero unresolved identities, and focused parser/domain/Planner/UI checks pass. The committed report owns D-055/D-057/D-071 deltas and passes 11/11 representative cases with 22 advisory outliers across 189 evaluations.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `npm run test:e2e -- --workers=1`                           | `pass`                                                                    | The 2026-07-11 production-preview gate passed 58/58 in Chromium after the bounded keyboard pass. It includes the prior 57 workflow/numeric cases plus skip-link, roving Workbench tablist/tabpanel, arrow/Home/End tab activation, Dense roving focus, visible computed focus outline and Enter target selection. The run used sandbox-external localhost permission because the managed sandbox rejects preview binds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
