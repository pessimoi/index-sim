...
Focused writer tests validate the normalized fixture contract and first-page Inertia item adapter used by `--upstream-url`: exact root/origin checks, sequential request delay, redirect/timeout/size/content failures, username removal, buy/sell inclusion, unweighted quantity, MAD and median-ratio outlier filtering, sparse/stale prior-price retention, missing/unknown/duplicate mapping gates, 90-day 12-hour plus older daily history retention, two-file idempotence and unchanged `alch.json`. A local dry-run fixture check needs no network:
...
The scheduled workflow runs only at 00:15 and 12:15 UTC, has no `workflow_dispatch`, requires `MARKET_PRICES_UPSTREAM_URL` to be the exact reviewed root, validates focused tests/JSON/diffs, rejects changes outside `prices.json` and `price-history.json`, and commits only real two-file diffs. Do not run the live workflow locally. Acceptable automated use and scheduled-run evidence belong in the operations evidence phase.
...
| `npm run test`                                              | `pass`                                                                    | 2026-07-10 full Vitest run passed across 33 files and 517 tests, including generated runtime, raw parser/generator, canonical economy, shared scheduled history and browser-local comparisons, Planner/loadout requirements, shareable permalink contract/adapter behavior, Duel export/import, legacy Duel migration, the all-monster Duel matrix and its maximum-size CPU-time bound, the 18-case rewrite browser-fixture adapter, item-page market writer/estimator behavior, the observed live Hiscores row shape, generated-price fallback composition, UI/view-model, persistence, migration and performance coverage.                                                                                                                                                                                                                                                                                                   |
...
| `npm run test:e2e`                                          | `pass`                                                                    | The latest full 2026-07-10 production-preview gate passed 57/57 in Chromium. It includes all 18 golden fixture setups, shared scheduled history without localStorage writes, local comparison save/clear isolation, generated alch for imported/legacy PriceSets and the existing workbench/migration/permalink workflows. Accepted generated-alch GP expectation changes were rerun in the complete gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
