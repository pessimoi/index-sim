...
### Specced global Undo visibility and targeted Reset follow-up

`Basic combat setup`, `Result summary`, `Cannon` and `Loot/economy summary`
remain `Valmis`. The proposed
[global Undo and targeted Reset contract](../technical/global-undo-visibility-targeted-reset-spec.md)
will keep the existing single session-local Undo visible across desktop,
compact landscape and normal-flow mobile/portrait layouts. It also covers all
seven accepted Active assumptions Reset classes and their mirrored pane entry
points with one changed/no-op/restore rule; Loot's two existing Undo paths stay
in place while Manual overrides, Cannon, scarce spot, safespot and hidden gear
tiers gain the missing recovery. This planned hardening adds no reset target,
history stack, persisted Undo, storage schema or calculation change.

### Specced monster-specific changes management follow-up

`Basic combat setup`, `Dense spreadsheet view`, `Monster comparison`,
`Cannon` and `Loot/economy summary` remain `Valmis`. The proposed
[monster-specific changes management contract](../technical/monster-specific-changes-management-spec.md)
will add one Settings union over the existing Custom setup, Cannon, Loot
action, Loot settings and Compare-hidden records. It provides source-backed
inventory, owner-specific Review navigation and review-gated one-monster
cleanup with complete Undo; it does not add a setup type, inline duplicate
editor, cross-monster wipe, storage schema or calculation path. Workspace and
Local state recovery retain file- and area-level ownership rather than serving
as this ordinary per-monster manager.

### Specced local price-history lifecycle management follow-up

`Loot/economy summary` and `Market price sync` remain `Valmis`. The proposed
[local price-history lifecycle contract](../technical/local-price-history-lifecycle-management-spec.md)
will expose every existing browser-local comparison, its `N/20` capacity and
the exact next replacement under Economy Price history. It adds review-gated
...
provenance, analysis formulas, Workspace transfer, provider policy and the
separate later date-formatting follow-up remain unchanged.

### Specced complete and actionable Planner notices follow-up
...
will preserve every distinct structured notice through the calculation Worker,
include start, displayed-step and training-stance warning contexts, expose the
existing truncated-plan truth as an issue and remove the current four-message
display cap. One native disclosure will distinguish issues from notes and
current output from a retained previous plan; each row routes by structured
...
### Specced saved setup Merge and rename safety follow-up

`Setup comparison` remains `Valmis`. The proposed
[saved setup Merge and rename safety contract](../technical/saved-setup-merge-rename-safety-spec.md)
will replace the current count-only incoming-ID-wins file Apply with a complete
...
recipient names must be resolved. Direct rename will use explicit Save/Cancel
instead of blur commit, while changed Merge/rename becomes one durability-aware
