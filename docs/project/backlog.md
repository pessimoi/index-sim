...
- `Blocked` - Choose production runtime, hosting and authoritative upstream for hiscores after the pending hiscores API answer is known. Market price refresh now has an accepted GitHub Actions scheduled static JSON model, but the writer implementation is still missing.
...
- `Open` - Implement the accepted game revision bump workflow: manual `npm run data:generate`, gitignored `.sources/lostcity-content/` checkout, branch/PR only, overwrite current source pin/snapshot/impact report, generate only simulator-consumed normalized domain data, generated snapshot diff, validation, merge-blocking hybrid representative suite, informational all-monster scan with fixed baseline/threshold policy and explicit accepted calculation deltas.
- `Open` - Implement the accepted GitHub Actions scheduled `markets.lostcity.rs` price writer: run at 00:15 and 12:15 UTC, generate `prices.json`, `alch.json` and retained 12-hour `price-history.json`, validate them and commit only real diffs with `GITHUB_TOKEN`.
...
- `Blocked` - Finish market price product acceptance after the authoritative `markets.lostcity.rs` response contract and GitHub Actions scheduled writer are implemented. The rewrite has a disabled-provider same-origin API, validated adapter, current-monster/all-supported UI flow, browser-local accepted-price history and a working-tree scheduled static snapshot candidate, but production must not enable user-triggered upstream refresh and the writer/upstream contract are still missing.
