...
- Finish any final Trip wording or exact-legacy-parity acceptance after the food/banking, scarce/AFK target/respawn, reserve summary, prayer restore capacity, general potion carry controls, domain-owned potion recommendation/apply flow, grouped Trip summary and Cannon sparse-linking paths. Prayer potion modeling, live data, canonical data and provider decisions remain outside this backlog item.
...
- Extend dense compare parity beyond the current monster/drop filters, irrelevant state, per-monster alch/overhead markers, visible-row XP/net-GP scale indicators and browser-rendered release-path numeric snapshots only if broader full browser-display parity is required.
