...
- No secret, account model, database or server-managed mutable market history is configured. The workflow requires `MARKET_PRICES_UPSTREAM_URL` to equal `https://markets.lostcity.rs/`; do not configure it before acceptable sequential automated use is confirmed.
...
- Local writer: `--input` validates normalized fixture data; `--upstream-url https://markets.lostcity.rs/` derives one `/items/{slug}` request per approved mapping. Requests are sequential with 350 ms spacing and per-page redirect/timeout/size/content checks. The adapter accepts only the first ten completed buy/sell rows and drops usernames. MAD/median filtering, unweighted observations and freshness gates update prices or retain prior values. Candidates are validated before deterministic writes to only `prices.json` and `price-history.json`.
...
Open question: acceptable sequential automated use still needs confirmation before setting the exact root `MARKET_PRICES_UPSTREAM_URL` and trusting scheduled-current claims. The item-page shape is implemented from bounded metadata-only evidence; no raw page or username is committed.
...
| Production live integrations | `partial decision`               | Hiscores and market paths have mocked same-origin coverage. Market now has the D-062 item-page adapter/estimator, generated alch ownership, shared static history and a two-file commit-if-diff workflow; acceptable automated use, root variable configuration and a verified successful run are still absent. D-053 keeps that evidence out of the trusted-tester gate but requires it before scheduled-current claims. Hiscores has the D-061 provider in dev/preview but still lacks production runtime/hosting.                                                                                                                                                                           | Do not describe live hiscores or scheduled-current market prices as production-available until their runtime/writer evidence passes. Keep user-triggered upstream market refresh out of production copy.                                                      |
