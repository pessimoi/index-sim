...
- `.github/workflows/update-market-prices.yml` is the only GitHub Actions workflow in this checkout. D-066 uses Cloudflare Workers Builds, not another GitHub workflow, for production build/deploy after external account connection.
...
- No secret, account model, database or server-managed mutable market history is configured. Public crawler policy and the full live dry-run are reviewed, and the Actions variable is configured as the exact approved `MARKET_PRICES_UPSTREAM_URL=https://markets.lostcity.rs/` root. The workflow now waits only for its first successful configured scheduled run.
...
- Cloudflare account/Git connection and deployed evidence remain pending; no custom domain is required for preview.
...
The latest release-evidence check was refreshed through 2026-07-11 for the source-backed Revision 274 Vite/React runtime. Typecheck, 523 unit tests, 19 golden tests, build, D-066 artifact validation, dependency audit and tooling-hygiene gates pass. The latest complete production-preview Playwright gate remains 57/57; the D-066 rerun was blocked before browser execution by managed-sandbox `listen EPERM`. All four raw source-impact diagnostics completed, and the committed generator report passes 10/10 representative cases.

| Area                         | Status                            | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Operational follow-up                                                                                                                                                                                                                                         |
| ---------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Core commands                | `pass`                            | The 2026-07-11 refresh ran typecheck, full unit (34 files/523 tests), golden (19), build, artifact validation, lint, format, audit and diff checks; all passed. The artifact report covered 7 files/2 hashed assets, 13 history snapshots and SHA-256 `a8bd9ee19cfa6c17ff659006cbde54e50ab846e2c93e276098fa2af35bf4d2ce`. The build emitted only the known chunk-size warning.                                                                                                                                                                                                                                                                                                                 | Re-run in Cloudflare Builds and before release.                                                                                                                                                                                                               |
...
| Production live integrations | `runtime implemented`             | Market has its D-062-D-064 workflow, live dry-run and exact repository variable; only a verified configured scheduled run remains before scheduled-current claims. Hiscores has the D-061 provider in Vite and the D-066 Cloudflare production Worker. Cloudflare account connection plus preview/production live and D-065 evidence remain.                                                                                                                                                                                                                                                                                                                                                   | Do not describe live Hiscores or scheduled-current market prices as production-available until their deployed/writer evidence passes. Keep user-triggered upstream market refresh out of production copy.                                                     |
| Deploy/security headers      | `Cloudflare implementation ready` | Node 22/npm 10, exact Wrangler commands, Worker-first API routing, Static Assets SPA fallback, `_headers`, disabled observability/Logpush, artifact validation and focused Worker/deployment tests are implemented. The current artifact passes with 7 files, 2 assets and checksum `a8bd9ee…d2ce`.                                                                                                                                                                                                                                                                                                                                                                                            | Connect Cloudflare Builds to `master`, run the repository build/deploy commands, verify account log settings and run `npm run deploy:smoke` against preview and production before release.                                                                    |
...
A later public release still requires Cloudflare account connection, deployed
