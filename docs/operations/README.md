...
Revision 274 runtime, scheduled market writer, same-origin Hiscores provider
...
| Market prices               | The D-062-D-064/D-085 scheduled writer, exact repository variable, validated artifacts and offline/import fallback are implemented.             | Observe and validate the first configured scheduled run before describing prices as scheduled-current.                                                           |
...
rollback rehearsal and configured cron observation are claim gates owned by an
adopter. They are not missing repository implementation under D-067.
